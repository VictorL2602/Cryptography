#include "include/caesar.h"   // Include Caesar cipher function declarations
#include "include/vigenere.h" // Include Vigenère cipher function declarations
...
int main(int argc, char *argv[]) { // Program entry point; argc = argument
  ...
  if (argc < 4) {                  // If too few arguments were given
  ...
  char *cipher = argv[1];  // The first argument after the program name (which
                           // cipher to use)
  ...
  if (strcmp(cipher, "caesar") == 0) { // If the chosen cipher is "caesar"
    ...
    if (strcmp(mode, "encrypt") == 0) { // If user chose encryption
      caesar_encrypt(msgRam);           // Call Caesar encryption function
      printf("Encrypted message: %s\n", msgRam); // Print the result
      return 0;                                  // Exit successfully

    } else if (strcmp(mode, "decrypt") == 0) { // If user chose decryption
      caesar_decrypt(msgRam); // Call Caesar decryption function
    ...
    if (strcmp(mode, "encrypt") == 0) { // If encrypt mode
      vigenere_encrypt(msgRam, key);    // Call Vigenère encryption function
      printf("Encrypted message: %s\n", msgRam); // Print encrypted text
      return 0;                                  // Exit successfully

    } else if (strcmp(mode, "decrypt") == 0) { // If decrypt mode
      vigenere_decrypt(msgRam, key); // Call Vigenère decryption function
